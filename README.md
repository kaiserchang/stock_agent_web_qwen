...
1. 點擊藍色的「**開始掃描**」按鈕
2. 系統會開始掃描台灣股市（預設掃描最近 2 個月的 20 支股票）
3. 掃描通常需要 2-3 分鐘完成
4. 完成後，結果會自動顯示在「投資建議名單」表格中
...
**A**: 這是正常的。系統需要為每支股票下載 60 天的歷史數據並進行技術分析。如果想加速，可以：
- 減少掃描股票數量（在掃描設定中調整）
- 縮短掃描時間範圍
...
**A**: 這是 Manus 發布環境的網絡限制。發布環境無法訪問外部 API。解決方案：
- 使用 CSV 上傳功能進行分析
- 在本機開發環境中執行掃描
